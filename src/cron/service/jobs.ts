...
export function assertSupportedJobSpec(job: CronJob): void {
...
export function recomputeNextRuns(state: CronServiceState): void {
  if (!state.store) {
    return;
  }
...
export function createJob(
  ...
  const job: CronJob = {
    id,
    agentId: normalizeOptionalAgentId(input.agentId) ?? null,
...
export function applyJobPatch(job: CronJob, patch: CronJobPatch): void {
  ...
  if (patch.payload) {
    job.payload = mergeCronPayload(job.payload, patch.payload as any);
  }
  if (patch.isolation) {
  ...
  if ("agentId" in patch) {
    job.agentId = normalizeOptionalAgentId(patch.agentId) ?? null;
  }

  assertSupportedJobSpec(job);
}

function mergeCronPayload(existing: CronPayload, patch: any): CronPayload {
...
function buildPayloadFromPatch(patch: any): CronPayload {
  if (patch.kind === "systemEvent") {
